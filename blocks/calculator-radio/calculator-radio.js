      ...
      const parseNode = (node) => {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          if (text) {
            ...
            if (lower === 'salaried' || lower === 'biz' || lower === 'business') {
              if (!foirType) foirType = lower === 'business' ? 'biz' : lower;
...
/**
 * Gets the product type from the block's CSS classes (set by `classes` field in model).
 * @param {Element} block - The calculator-radio block
 * @returns {string}
 */
function getProductType(block) {
  const validTypes = ['hl', 'pl', 'bl', 'ubl'];
  const found = validTypes.find((t) => block.classList.contains(t));
  return found || 'hl';
}
