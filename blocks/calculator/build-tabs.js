...
export function buildCalculatorParent(description, tabNames, ctaItems, blocks, isGstCalculator = false) {
  ...
  if (ctaItems.length > 0) {
    ...
    ctaItems.forEach((serverAnchor, idx) => {
      ...
      if (isFormTrigger) {
        serverAnchor.addEventListener('click', async (e) => {
          e.preventDefault();
          try {
            const { onCLickApplyFormOpen, formOpen } = await import('../applyloanform/applyloanforms.js');
            const mainContainer = serverAnchor.closest('main') || document.querySelector('main') || document.body;
            ...
            console.warn('Calculator CTA: could not open loan form', err);
          }
        });
      }
...
export function initCalculatorTabs(section = null) {
  ...
  headingtabs.addEventListener('click', (e) => {
