/* eslint-disable no-use-before-define, no-console, no-nested-ternary, consistent-return, no-promise-executor-return, no-restricted-globals, func-names, no-unused-vars, max-len, no-plusplus */
...
function isAuthorEnvironment() {
  ...
  return hostname.includes('author-p') || pathname.includes('/editor.html/');
}
 ...
 * logic expects. Keys are lowercase with spaces/dashes/underscores stripped.
 */
const INPUT_NAME_MAP = {
  ...
  loanoriginationcharges: 'origincharges',
};
...
function combineSection(section) {
  ...
  if (isAuthorEnvironment()) {
    calcWrappers.forEach((w) => { w.style.display = 'none'; });
    dcwToRemove.forEach((dcw) => { dcw.style.display = 'none'; });
  } else {
...
export default async function decorate(block) {
  ...
  // Read calculator type from CSS class (classes_type field), not from content
  ...
  const calcName = validCalcTypes.find((t) => block.classList.contains(t)) || '';
  ...
  // Structure (content only, no productType/calcName): tabLabel, description, output1Image, output1Label, ...
  ...
  let allTextAfterImage = imageIndex >= 0
    ...
    : firstRowArray.filter((el) => el.textContent?.trim() && !el.querySelector('img')); // All text elements (no productType/calcName in content)
  ...
  Array.from(block.children).slice(1).forEach((r) => {
    ...
    // Skip if the authored name doesn't map to a known input type
    // (prevents random content from being treated as a slider)
    if (!INPUT_NAME_MAP[(authoredName || '').toLowerCase().replace(/[\s_-]/g, '')]) return;
