...
async function loadBlock(block) {
  ...
  if (status !== 'loading' && status !== 'loaded') {
    ...
    try {
        const categoryTemplate = window.getMetadata('category-template');

      const cssLoaded = loadCSS(`${window.hlx.codeBasePath}/blocks/${categoryTemplate}/${blockName}/${blockName}.css`);
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            const mod = await import(
              `${window.hlx.codeBasePath}/blocks/${categoryTemplate}/${blockName}/${blockName}.js`
            );
            if (mod.default) {
